...
INSERT INTO invoicing_olap.dim_product (product_id, product_name, product_description, unit_price, vat_rate, updated_at)
SELECT p.product_id, p.product_name, p.product_description, p.unit_price, p.vat_rate, p.updated_at
FROM invoicing.clients p, w WHERE p.updated_at >= w.last_loaded_ts
ON CONFLICT (client_id) DO UPDATE SET product_name=EXCLUDED.product_name, product_description=EXCLUDED.product_description,
